...
def train_target(args):
    ...
    elif args.net == 'datmamba':
        netF = network.datmamba().cuda()
    netB = network.feat_bootleneck(type=args.classifier, feature_dim=netF.in_features, bottleneck_dim=args.bottleneck).cuda()
    netC = network.feat_classifier(type=args.layer, class_num = args.class_num, bottleneck_dim=args.bottleneck).cuda()

    modelpath = args.output_dir_src + '/source_F.pt'   
    netF.load_state_dict(torch.load(modelpath))
    # total_params = sum(p.numel() for p in netF.parameters())
    # print("netF parameters:", total_params)
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netF.eval()
            netB.eval()
            mem_label, dd = obtain_label(dset_loaders['test'], netF, netB, netC, args, ent_old_val)
        ...
        sample_outputs = F.softmax(outputs_test, dim=1)  # (batch_size, num_classes)
        pred_soft = dd[tar_idx]
        pred_soft = torch.tensor(pred_soft, dtype=torch.float32).cuda()
        if sample_outputs.size(1) != pred_soft.size(1):
            min_size = min(sample_outputs.size(1), pred_soft.size(1))
            sample_outputs = sample_outputs[:, :min_size]
            pred_soft = pred_soft[:, :min_size]


        kl_loss = F.kl_div(F.log_softmax(sample_outputs, dim=1), pred_soft, reduction='batchmean')
        classifier_loss += kl_loss *0.1
...
def obtain_label(loader, netF, netB, netC, args, ent_old_val_):
    ...
    # args.out_file.flush()
    # print(log_str+'\n')
    avg_entropy = torch.mean(ent)
    smooth_factor = 0.1 * avg_entropy / (torch.log(torch.tensor(all_output.size(1))) + 1e-8)
    smooth_factor = torch.clamp(smooth_factor, 0.01, 0.3)
    soft_labels = (1 - smooth_factor) * all_output + smooth_factor / args.class_num
    for round in range(1):
        #aff = np.eye(K)[pred_label]
        aff = soft_labels.cpu().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--net', type=str, default='datmamba', help="alexnet, vgg16, resnet50, res101")
